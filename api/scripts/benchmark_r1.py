...
def execute_shell_script():
    result = subprocess.run(['bash', './gen_random_get_patient.sh'], capture_output=True, text=True)
...
def send_curl_request(data):
    ...
    try:
        ...
        subprocess.run(["curl", "--silent", "-o", "/dev/null", "-X", "GET", "-H", "Content-Type: application/json", "-d", data, "http://172.20.0.2:8080/v1/patient"], check=True)
