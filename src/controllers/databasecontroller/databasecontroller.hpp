...
public:
  ...
  std::optional<bool> checkItemExists(const std::string &table,
                                      const std::string &column,
                                      const std::string &value);
  std::optional<std::string>
  getPasswordHashForUserID(const uint64_t &user_id,
...
private:
  ...
  std::optional<R> executer(const F &f, Args &&...args) {
    ...
    try {
      ...
      std::optional<R> results =
          std::invoke(f, db.get(), std::forward<Args>(args)...);
