...
Server::Server(uint16_t srv_threads, uint16_t db_connections)
    ...
    , app(std::make_shared<crow::App<crow::CORSHandler, ElapsedTime, Authentication, Authorization, Search, DataIntegrity>>(*elapsedTime, *authentication, *authorization, *search, *dataIntegrity))
