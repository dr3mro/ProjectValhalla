...
Server::Server(uint16_t srv_threads, uint16_t db_connections)
    ...
    , getpatientverifier(std::make_shared<GetPatientVerifier>())
    , app(std::make_shared<crow::App<crow::CORSHandler, ElapsedTime, Authentication, Authorization, Search, DataIntegrity, GetPatientVerifier>>(*elapsedTime, *authentication, *authorization, *search, *dataIntegrity, *getpatientverifier))
