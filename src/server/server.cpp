...
Server::Server(uint16_t srv_threads, uint16_t db_connections)
    ...
    , userController(std::make_shared<UserController>(dbController, restHelper, tokenizer, passwordCrypt))
    , providerController(std::make_shared<ProviderController>(dbController, restHelper, tokenizer, passwordCrypt))
