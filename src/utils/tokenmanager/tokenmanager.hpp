 ...
 *
 * The TokenManager class is responsible for generating and validating JWT tokens for user authentication. It uses the jwt-cpp library to handle the token
 ...
 *
 * The TokenManager class also has a `TokenManagerParameters` struct that holds the configuration parameters for the token management, such as the issuer, type,
 * validity, and secret.
 *
